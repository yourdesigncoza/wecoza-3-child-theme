...
define("WECOZA_THEME_VERSION", "6.1.1");
...
function enqueue_assets()
{
    ...
    wp_enqueue_style(
        "ydcoza-bootstrap-demo",
        WECOZA_CHILD_URL . "/includes/css/ydcoza-bootstrap-demo.css",
        ["main"],
    ...
    wp_enqueue_style(
        "ydcoza_theme-css",
        WECOZA_CHILD_URL . "/includes/css/ydcoza-theme.css",
        ["ydcoza-bootstrap-demo"],
        WECOZA_PLUGIN_VERSION,
    );
    ...
    wp_localize_script("wecoza-table-handler", "wecoza_table_ajax", [
        "ajax_url" => admin_url("admin-ajax.php"),
        "nonce" => wp_create_nonce("wecoza_table_nonce"),
    ]);
    wp_localize_script("jquery", "wecoza_ajax", [
        "ajax_url" => admin_url("admin-ajax.php"),
    ]);
}
...
function wecoza_page_needs_forms(): bool
{
    ...
    if (
        $post &&
        (has_shortcode($post->post_content, "wecoza_form") ||
...
function ydcoza_load_child_style_last()
{
    ...
    wp_enqueue_style(
        "ydcoza_styles-css",
        WECOZA_CHILD_URL . "/includes/css/ydcoza-styles.css",
        [],
        WECOZA_PLUGIN_VERSION,
    );
    // app - deferred to avoid blocking render
    wp_enqueue_script(
        "wecoza-table-handler",
        WECOZA_CHILD_URL . "/includes/js/app.js",
        ["jquery"],
        WECOZA_PLUGIN_VERSION,
        ["in_footer" => true, "strategy" => "defer"],
    );
...
function ydcoza_add_resource_hints()
{
    ...
    $dns_prefetch_origins = ["//fonts.googleapis.com", "//fonts.gstatic.com"];
